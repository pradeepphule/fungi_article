...
dim(gene_expression)
transcript_expression <- as.data.frame(texpr(bg_filt, meas='FPKM'))
head(transcript_expression)
dim(transcript_expression)

data_colors=c("tomato1","tomato2","tomato3","cyan1","cyan2","cyan3")
...
#i = row.names(gene_expression) == "MSTRG.3"
#gene_expression[i,]
...
#min(gene_expression[,"FPKM.NoSalt_rep2"])
#max(gene_expression[,"FPKM.NoSalt_rep2"])
...
data_columns=c(1:6)
short_names=c("Salt_r1", "Salt_r2", "Salt_r3","NoSalt_r1", "NoSalt_r2","NoSalt_r3")
...
boxplot(log2(gene_expression[,data_columns]+min_nonzero), col=data_colors, names=short_names, las=2, ylab="log2(FPKM)", main="Gene abundance distribution ")
boxplot(log2(transcript_expression[,data_columns]+min_nonzero), col=data_colors, names=short_names, las=2, ylab="log2(FPKM)", main="Transcript abundance distribution ")
...
#which fun. identifies row no. fitting to condition
sig=which(results_genes$pval<0.05)
head(sig)
results_genes[,"log(fc)"] = log2(results_genes[,"fc"])
head(results_genes)

hist(results_genes[sig,"log(fc)"], breaks=50, col="seagreen", xlab="log2(Fold change) SaltVsNOSalt", main="Distribution of differential expression values")
...
gene_expression[,"Salt"]=apply(gene_expression[,c(1:3)], 1, mean)
gene_expression[,"NoSalt"]=apply(gene_expression[,c(4:6)], 1, mean)
head(gene_expression)
x=log2(gene_expression[,"Salt"]+min_nonzero)
y=log2(gene_expression[,"NoSalt"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="Salt FPKM (log2)", ylab="NoSalt FPKM (log2)", main="SaltVsNOSalt FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
...
sigde = which(abs(sigp[,"log(fc)"]) >= 2)
sig_tn_de = sigp[sigde,]

#Order the output by or p-value and then break ties using fold-change
#log(fc)greater or equal to 2 
o = order(sig_tn_de[,"qval"], -abs(sig_tn_de[,"log(fc)"]), decreasing=FALSE)
head(o)
output = sig_tn_de[o,c("gene_name","id","fc","pval","qval","log(fc)")]
head(output)
